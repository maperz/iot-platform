...
{
    if (!answer->valid || strlen(answer->name_buffer) == 0 || foundAddress == true)
    {
        return;
    }
    ...
    if (answer->rrtype == MDNS_TYPE_SRV && host.serviceName == answer->name_buffer)
    {
    ...
    {
        foundAddress = true;
        log(LogLevel::Info, "ServiceDiscovery: mqtt Broker discovered at %s:%d\n", host.address.c_str(), host.port);
    }
}
