...
class ConnectionInfoPage extends StatelessWidget {
  ...
  const ConnectionInfoPage({required this.connectionService, Key? key})
      : super(key: key);
  ...
  List<Widget> _createStatusWidget(ConnectionInfo? info) {
    const commonStyle = TextStyle(fontWeight: FontWeight.w500, fontSize: 18);

    final notConnectedText = Text("Not connected",
        style: commonStyle.copyWith(color: const Color(0xFFFF725F)));
    ...
    final connectedText = Text("Connected",
        style: commonStyle.copyWith(color: const Color(0xFFA5FF5F)));
